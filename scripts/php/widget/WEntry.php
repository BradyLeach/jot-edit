...
class WEntry extends CDiv {
    
    private $m_form;
    
    public function __construct($id='entry', $attributes=array('class'=>'widget'), $open=FALSE) {
        parent::__construct($id.'_widget', $attributes);
        
        $this->m_form = new FLogin();
        
        //if the form was not successfully processed prepare redisplay.
    ...
    private function InitWidgetButton(){
        ...
        $this->AddChild(new CLink('loginLink', array('class'=>'button'), 'sign in', '/jot-edit/action/login'));
    }
    
    
    private function InitWidgetCallOut($open){
            //Create the call out box
        $loginBox = new PCallout('loginBox');
